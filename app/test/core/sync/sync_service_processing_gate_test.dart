...
main() {
  group(
    'SyncService processing gate - Brain E2EE',
    () {
      ...
      setUpAll(
        () async {
          ...
          expect(
            ...
            isTrue,
            reason: 'lib/app/dependencies/app_dependencies.dart não foi encontrado.',
          );
      ...
      test(
        'SyncService possui processingGate',
        () {
          expect(
            syncServiceSource,
            contains(
      ...
      test(
        'syncNow processa somente itens retornados por _loadProcessableItems',
        () {
          final syncNowStart = syncServiceSource.indexOf(
            'syncNow({',
          );
          ...
          expect(
            ...
            reason: 'syncNow precisa obter os itens através de _loadProcessableItems().',
          );
          ...
          expect(
            ...
            reason: 'O handler só pode ser consultado depois que o gate filtrou os itens.',
          );
        },
      );
      ...
      test(
        ...
        () {
          ...
          final gateMethod = syncServiceSource.substring(
            methodStart,
            methodEnd,
          );

          final blockedLogIndex = gateMethod.indexOf(
            'ADIADO PELO GATE:',
          );

          final continueIndex = gateMethod.indexOf(
            'continue;',
            blockedLogIndex,
          );

          final resultAddIndex = gateMethod.indexOf(
            'result.add(',
          );
          ...
          expect(
            ...
            reason: 'O item bloqueado precisa ser descartado do lote antes de result.add().',
          );
        },
      );

      test(
        'gate não marca item bloqueado como sucesso ou falha',
        () {
          final methodStart = syncServiceSource.indexOf(
            '_loadProcessableItems() async',
          );
      ...
      test(
        'Brain E2EE registra gate baseado no BrainDataModeService',
        () {
          ...
          expect(
            brainHandler,
            contains(
              'brainDataModeService.canUseCloudSync',
            ),
          );
      ...
      test(
        'handler legado brain_review não é mais registrado',
        () {
          expect(
      ...
      test(
        'fila legada do Brain é purgada sem remover brain_e2ee_object',
        () {
