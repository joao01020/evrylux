...
class BrainNoteVaultStore {
  ...
  Future<BrainVaultObject> saveNote(
    ...
    final existing = await findEncryptedObjectByPath(note.path);
    ...
    if (decoded != null && _mapsEquivalent(decoded.data, data)) {
      return existing;
    }
  ...
  Future<List<BrainFile>> loadNotes() async {
    ...
    for (final object in objects) {
      ...
      try {
        result.add(_fromVaultData(decoded.data));
      } catch (_) {
  ...
  Future<BrainFile?> getNoteByPath(String path) async {
    ...
    final object = await findEncryptedObjectByPath(cleanPath);
    ...
    return _fromVaultData(decoded.data);
  }
  ...
  Future<BrainVaultObject?> findEncryptedObjectByPath(String path) async {
    ...
    for (final object in objects) {
      ...
      final objectPath = decoded.data['path']?.toString().trim() ?? '';

      if (objectPath == cleanPath) {
        return object;
      }
    }
