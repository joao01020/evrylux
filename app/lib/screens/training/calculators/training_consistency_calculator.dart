...
class TrainingConsistencyCalculator {
  ...
  static int expectedTrainingsUntilToday({
    ...
    final today = TrainingDateHelper.normalize(now);

    DateTime currentDate = DateTime(today.year, today.month, 1);

    int expected = 0;

    while (!currentDate.isAfter(today)) {
      if (normalizedWeekdays.contains(currentDate.weekday)) {
        expected++;
      }
  ...
  static double consistencyIndex({
    ...
    final result = completed / expected;
