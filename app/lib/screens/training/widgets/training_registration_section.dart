...
class TrainingRegistrationSection extends StatelessWidget {
  ...
  int get minutes {
    return controller.currentSeconds ~/ 60;
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const Text('Você pode selecionar mais de uma opção.'),

        const SizedBox(height: 12),

        _ActivityOptions(controller: controller),
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            onPressed:
                controller.selectedActivities.isEmpty || controller.isSaving
                ? null
                : onComplete,
            ...
            label: Text(
              controller.isSaving ? 'Salvando...' : 'Registrar treino',
            ),
          ),
        ),
...
class _ActivityOptions extends StatelessWidget {
  ...
  const _ActivityOptions({required this.controller});
  ...
  Widget build(BuildContext context) {
    return Wrap(
      spacing: 10,
      runSpacing: 10,
      children: TrainingController.activityOptions.map((activity) {
        return SizedBox(
          width: 105,
          child: ActivityCard(
            activity: activity,
            selected: controller.isActivitySelected(activity),
            onTap: () {
...
class _SelectedActivitiesCard extends StatelessWidget {
  final String text;
  final VoidCallback onClear;

  const _SelectedActivitiesCard({required this.text, required this.onClear});
