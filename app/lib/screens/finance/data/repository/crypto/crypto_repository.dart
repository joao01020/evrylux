...
class CryptoRepository {
  ...
  Future<List<CryptoTransactionModel>> load({bool forceRefresh = false}) async {
    ...
    try {
      ...
      final transactions = decoded
          .map(
            (item) => CryptoTransactionModel.fromMap(
  ...
  Future<void> save(List<CryptoTransactionModel> transactions) async {
    ...
    final encodedTransactions = jsonEncode(
      transactionsCopy.map((transaction) => transaction.toMap()).toList(),
    );
  ...
  Future<bool> delete(String id) async {
    ...
    final removed = transactions.length != initialLength;
  ...
  Future<List<CryptoTransactionModel>> bySymbol(String symbol) async {
    ...
    final filteredTransactions = transactions.where((transaction) {
      return transaction.symbol.trim().toUpperCase() == normalizedSymbol;
    }).toList();
  ...
  Future<double> totalInvested(String symbol) async {
    ...
    return transactions
        ...
        .fold<double>(0.0, (total, transaction) {
          return total + transaction.invested;
        });
  }
