...
class FinanceProjectionCalculator {
  ...
  /// 10 anos = 120 meses.
  int get totalMonths {
    if (model.projectionYears <= 0) {
      return 0;
    }

    return model.projectionYears * 12;
  }
  ...
  double get missingMoney {
    final difference = model.investmentGoal - model.patrimony;
  ...
  bool get goalReached {
    ...
    return model.patrimony >= model.investmentGoal;
  }
  ...
  double get realFinancialProgress {
    ...
    final progress = model.patrimony / model.investmentGoal;
  ...
  double get realFinancialPercentage {
    return realFinancialProgress * 100;
  }
  ...
  double get historyTotalValue {
    return history.fold<double>(0.0, (total, item) => total + item.value);
  }
  ...
  double get averageContribution {
    ...
    return historyTotalValue / history.length;
  }
  ...
  double get historyObjectiveProgress {
    final total = history.fold<double>(
      0.0,
      (progress, item) => progress + item.objectiveProgress,
    );
  ...
  double get historyTimeProgress {
    final total = history.fold<double>(
      0.0,
      (progress, item) => progress + item.timeProgress,
    );
  ...
  double get objectiveBar {
    ...
    final progress = financialProgress > historicalProgress
        ? financialProgress
        : historicalProgress;
  ...
  double get timeRemainingBar {
    return (1 - objectiveBar).clamp(0.0, 1.0).toDouble();
  }

  /// Tempo restante em porcentagem.
  double get timeRemainingPercentage {
    return timeRemainingBar * 100;
  }
  ...
  double objectiveBarAfterIncrement(double increment) {
    if (increment <= 0) {
      return objectiveBar;
    }

    final futureProgress = objectiveBar + increment;

    return futureProgress.clamp(0.0, 1.0).toDouble();
  }

  /// Calcula a barra do tempo restante
  /// depois de aplicar um incremento.
  double timeRemainingBarAfterIncrement(double increment) {
    final futureObjective = objectiveBarAfterIncrement(increment);

    return (1 - futureObjective).clamp(0.0, 1.0).toDouble();
  }
  ...
  double futurePatrimony(double monthlyContribution) {
    if (monthlyContribution <= 0) {
      return model.patrimony;
    }

    return model.patrimony + (monthlyContribution * totalMonths);
  }

  /// Patrimônio previsto no ritmo tranquilo.
  double get tranquilFuturePatrimony {
    return futurePatrimony(model.minimumGoal);
  }

  /// Patrimônio previsto no ritmo normal.
  double get normalFuturePatrimony {
    return futurePatrimony(model.mediumGoal);
  }

  /// Patrimônio previsto no ritmo forte.
  double get strongFuturePatrimony {
    return futurePatrimony(model.maximumGoal);
  }

  /// Percentual que o patrimônio futuro representa
  /// em relação ao objetivo.
  double futureObjectiveProgress(double monthlyContribution) {
    if (model.investmentGoal <= 0) {
      return 0;
    }

    final future = futurePatrimony(monthlyContribution);

    return (future / model.investmentGoal).clamp(0.0, 1.0).toDouble();
  }
  ...
  int remainingMonths(double monthlyContribution) {
    if (goalReached) {
      return 0;
    }
    ...
    return (missingMoney / monthlyContribution).ceil();
  }

  /// Quantos anos inteiros faltam.
  int remainingYears(double monthlyContribution) {
    return remainingMonths(monthlyContribution) ~/ 12;
  }

  /// Quantos meses restam depois dos anos inteiros.
  int remainingExtraMonths(double monthlyContribution) {
    return remainingMonths(monthlyContribution) % 12;
  }
  ...
  int get estimatedRemainingMonths {
    ...
    if (averageContribution > 0) {
      return remainingMonths(averageContribution);
    }

    if (model.minimumGoal > 0) {
      return remainingMonths(model.minimumGoal);
    }
  ...
  int get estimatedRemainingYears {
    return estimatedRemainingMonths ~/ 12;
  }
  ...
  int get normalRemainingMonths {
    return remainingMonths(model.mediumGoal);
  }

  /// Tempo do ritmo forte.
  int get strongRemainingMonths {
    return remainingMonths(model.maximumGoal);
  }
  ...
  String estimatedTime(double monthlyContribution) {
    if (model.investmentGoal <= 0) {
    ...
    final months = remainingMonths(monthlyContribution);
  ...
  String get estimatedRemainingTimeText {
    ...
    if (estimatedRemainingMonths <= 0) {
      return 'Sem previsão';
    }

    return formatMonths(estimatedRemainingMonths);
  }

  /// Texto do tempo no ritmo tranquilo.
  String get tranquilTimeText {
    return estimatedTime(model.minimumGoal);
  }

  /// Texto do tempo no ritmo normal.
  String get normalTimeText {
    return estimatedTime(model.mediumGoal);
  }

  /// Texto do tempo no ritmo forte.
  String get strongTimeText {
    return estimatedTime(model.maximumGoal);
  }
  ...
  DateTime estimatedDate(
    ...
    return addMonths(referenceDate ?? DateTime.now(), months);
  }

  /// Data prevista baseada no histórico.
  DateTime estimatedCompletionDate({DateTime? referenceDate}) {
    return addMonths(referenceDate ?? DateTime.now(), estimatedRemainingMonths);
  }

  /// Data prevista no ritmo tranquilo.
  DateTime tranquilCompletionDate({DateTime? referenceDate}) {
    return estimatedDate(model.minimumGoal, referenceDate: referenceDate);
  }

  /// Data prevista no ritmo normal.
  DateTime normalCompletionDate({DateTime? referenceDate}) {
    return estimatedDate(model.mediumGoal, referenceDate: referenceDate);
  }

  /// Data prevista no ritmo forte.
  DateTime strongCompletionDate({DateTime? referenceDate}) {
    return estimatedDate(model.maximumGoal, referenceDate: referenceDate);
  }
  ...
  int savedMonthsComparedToTranquil(double contribution) {
    if (contribution <= 0) {
      return 0;
    }
    ...
    final tranquilMonths = remainingMonths(model.minimumGoal);

    final contributionMonths = remainingMonths(contribution);

    final difference = tranquilMonths - contributionMonths;
  ...
  double savedYears(double contribution) {
    return savedMonthsComparedToTranquil(contribution) / 12;
  }

  /// Texto sobre o tempo conquistado.
  String savedTimeText(double contribution) {
    final months = savedMonthsComparedToTranquil(contribution);
  ...
  String formatMonths(int totalMonths) {
    if (totalMonths <= 0) {
      return 'Objetivo alcançado';
    }
    ...
    final monthText = months == 1 ? '1 mês' : '$months meses';
  ...
  DateTime addMonths(DateTime date, int monthsToAdd) {
    ...
    final validDay = date.day > lastDayOfMonth ? lastDayOfMonth : date.day;
