...
class _EvryluxAppState extends State<EvryluxApp> {
  ...
  void _startupLog(String message) {
    debugPrint(
      '[STARTUP][APP] +${_startupWatch.elapsedMilliseconds}ms $message',
    );
  }
  ...
  void initState() {
    ...
    // cache, rede, reminders, presença e updates só começa depois do
    // primeiro frame. Isso evita disputar o isolate/I/O com o build inicial.
    ...
    _authSubscription = supabaseClient.auth.onAuthStateChange.listen((
      ...
      if (isAuthenticated) {
        if (_firstFrameRendered) {
          ...
          _startupLog('tarefas secundárias aguardando primeiro frame');
        }
      } else if (mounted) {
  ...
  void _startDeferredStartupTasks() {
    if (!_firstFrameRendered || _deferredStartupStarted) {
      return;
    }

    _deferredStartupStarted = true;
    _startupLog('tarefas secundárias pós-frame iniciadas');
  ...
  Future<void> _onReminderDue(ReminderModel reminder) async {
    ...
    try {
      final _ReminderAction? result = await showDialog<_ReminderAction>(
        context: context,
      ...
      if (result == _ReminderAction.complete) {
        final bool success = await reminderController.complete(reminder.id);
  ...
  Future<void> _loadProfile() {
    final existing = _profileLoadInFlight;
    if (existing != null) {
      _startupLog('loadProfile já em andamento; reutilizando');
      return existing;
    }

    late final Future<void> future;
    future = _loadProfileInternal().whenComplete(() {
  ...
  Future<void> _loadProfileInternal() async {
    ...
    _startupLog('loadProfile iniciado');

    UserProfile? cachedProfile;

    try {
      final cacheStage = Stopwatch()..start();
      cachedProfile = await _profileRepository.getCachedCurrentProfile();

      final String? cachedEmail = await _profileRepository
          .getCachedCurrentEmail();
      _startupLog('cache do perfil (${cacheStage.elapsedMilliseconds}ms)');
    ...
    try {
      final remoteStage = Stopwatch()..start();
      final UserProfile? profile = await _profileRepository
          .refreshCurrentProfile();
      _startupLog('perfil remoto (${remoteStage.elapsedMilliseconds}ms)');
    ...
    _startupLog(
      'loadProfile concluído (${profileWatch.elapsedMilliseconds}ms)',
    );
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: AuthGate(
        ...
        authenticatedBuilder: (BuildContext context, user) {
          return WelcomeScreen(controller: widget.evolutionController);
        },
      ),

      routes: {
        '/study': (BuildContext context) {
        ...
        child: InkWell(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              shape: BoxShape.circle,
              border: filled
                  ? Border.all(color: _primary.withValues(alpha: .28))
                  : null,
            ),
            child: loading
                ? const SizedBox(
...
class _ReminderTime extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String hour = _twoDigits(date.hour);

    final String minute = _twoDigits(date.minute);
