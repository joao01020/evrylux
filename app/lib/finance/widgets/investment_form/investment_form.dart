...
class InvestmentForm extends StatefulWidget {
  ...
  @override
  State<InvestmentForm> createState() {
...
class _InvestmentFormState extends State<InvestmentForm> {
  ...
  @override
  void didUpdateWidget(covariant InvestmentForm oldWidget) {
    ...
    final controllersChanged =
        oldWidget.investedController != widget.investedController ||
        oldWidget.minimumController != widget.minimumController ||
        oldWidget.mediumController != widget.mediumController ||
        oldWidget.maximumController != widget.maximumController ||
        oldWidget.yearsController != widget.yearsController;
  ...
  double get _currentPatrimony {
    return InvestmentFormFormatter.parseMoney(widget.investedController.text);
  }

  double get _minimumGoal {
    return InvestmentFormFormatter.parseMoney(widget.minimumController.text);
  }

  double get _mediumGoal {
    return InvestmentFormFormatter.parseMoney(widget.mediumController.text);
  }

  double get _maximumGoal {
    return InvestmentFormFormatter.parseMoney(widget.maximumController.text);
  }

  int get _projectionYears {
    return InvestmentFormFormatter.parseYears(widget.yearsController.text);
  }
  ...
  void _handleSave() {
    ...
    if (!_validation.isValid) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _validation.message ?? 'Preencha corretamente o planejamento.',
          ),
        ),
      );
    ...
    return Column(
      children: [
        InvestmentProjectionCard(
          ...
          description:
              'Um caminho leve para manter a constância mesmo nos meses mais difíceis.',
          monthlyContribution: _minimumGoal,
          contributed: _calculator.totalContributed(_minimumGoal),
          projectedPatrimony: _calculator.projection(_minimumGoal),
          projectionYears: _projectionYears,
        ),
        ...
        InvestmentProjectionCard(
          ...
          title: 'Ritmo normal',
          description:
              'Um caminho equilibrado entre conforto financeiro e velocidade.',
          monthlyContribution: _mediumGoal,
          contributed: _calculator.totalContributed(_mediumGoal),
          projectedPatrimony: _calculator.projection(_mediumGoal),
          projectionYears: _projectionYears,
        ),
        ...
        InvestmentProjectionCard(
          ...
          title: 'Ritmo forte',
          description:
              'Um caminho acelerado para aproximar o objetivo mais rapidamente.',
          monthlyContribution: _maximumGoal,
          contributed: _calculator.totalContributed(_maximumGoal),
          projectedPatrimony: _calculator.projection(_maximumGoal),
          projectionYears: _projectionYears,
        ),
      ],
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            InvestmentValidationBox(message: _validation.message),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: _validation.isValid ? _handleSave : null,
